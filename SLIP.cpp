...
int SLIPDecoder::addByte(char *b)
{
    ...
    syslog(LOG_INFO, "addByte a");
    // and END packet indicates the end of a message
    if (*b == END) {
        syslog(LOG_INFO, "addByte aa");
        retVal = _msgLen;
        syslog(LOG_INFO, "addByte aaa");

        // Reset the _msgLen for the next message
        reset();
        syslog(LOG_INFO, "addByte reset");

        return retVal;
    }

    syslog(LOG_INFO, "addByte b");
    // Error check
    if (_msgLen > _maxLen) {
        reset();
        return -1;
